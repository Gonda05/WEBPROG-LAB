...
$sortField = $_GET['sort'] ?? 'nim'; // default sort field
$sortDir = strtolower($_GET['dir'] ?? 'asc');
$sortDir = ($sortDir === 'desc') ? 'DESC' : 'ASC'; // secure the direction

$searchField = $_GET['field'] ?? '';
$searchKeyword = $_GET['keyword'] ?? '';

// Whitelist to prevent SQL injection
$allowedFields = ['nim', 'nama', 'jurusan', 'tahun_masuk', 'alamat', 'telp', 'user_input', 'tanggal_input'];
$whereClause = '';
...
$query = "SELECT * FROM mahasiswa $whereClause ORDER BY $sortField $sortDir";
$result = mysqli_query($conn, $query);
?>

<!-- Trigger buttons -->
<h1 class="text-3xl font-semibold mb-2">Data Mahasiswa</h1>
...
<form class="flex-grow min-w-[200px]" method="GET" action="">
        ...
        <div class="flex w-full">
            ...
            <div class="relative flex-grow flex flex-row">
                <input
                    ...
                    placeholder="Cari data mahasigma..."
                    required
                />
    ...
    <button
        ...
        onclick="openModal({ title: 'Tambah Mahasiswa', url: '/menu/mahasiswa/tambah.php' })"
    >
        Tambah Mahasiswa
    </button>
</div>
...
<?php
render_table($result, [
    'Edit' => 'javascript:void(0)" onclick="openModal({ title: \'Edit Mahasiswa\', url: \'/menu/mahasiswa/edit.php?nim={nim}\' })',
    'Hapus' => '/menu/mahasiswa/hapus.php?nim={nim}'
]);
?>
